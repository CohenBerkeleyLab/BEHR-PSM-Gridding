...
behr_datasets = ['TiledArea', 'TiledCornerLatitude', 'TiledCornerLongitude',
                 ...
                 'vcdQualityFlags', 'XTrackQualityFlags', 'SolarZenithAngle', 'Time']
    ...
    with open(vers_file, 'r') as f:
        ...
        for line in f:
            ...
            if 'ver_str' in line:
                m = re.search('(?<=\').*(?=\')', line)
                if m is None:
                    raise ValueError('Could not find the version string (encompassed by single quotes) in the line containing "vers_str"')
                else:
                    return m.group()



def behr_preprocessing(BEHRColumnAmountNO2Trop, ColumnAmountNO2TropStd, SolarZenithAngle, vcdQualityFlags,
    ...
    # mask bad values - BEHR does not have its own std. deviation value
    ...
    area = FoV75Area.reshape(1, FoV75Area)
...
def grid_day_from_file(behr_file, grid_info):
    ...
    for _, _, data in omi.he5.iter_behr_orbits_in_file(behr_file, behr_datasets):
